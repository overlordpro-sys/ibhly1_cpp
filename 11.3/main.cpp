...
void addDice(int &player, int &other1, int &other2, bool &forfeit, bool &done,
             int dice);

using std::cout, std::endl, std::cin, std::string, std::vector, std::to_string,
    std::setw;
...
int main() {
   ...
   for (char game = '1'; game - '0' <= 5; game++) {
      ...
      cout << "Input the dice rolls for game " << game
           << " separated by a comma and terminating with 0: ";
      cin >> scanner;
      input += "Line #";
      input += game;
      input += ": " + scanner;
   ...
   for (int i = 0; i < in_list.size(); i++) {
      cout << setw(50) << std::left << in_list.at(i) << setw(40) << std::left
           << out_list.at(i) << "\n";
   }
   return 0;
}
